 ...
 * gcc -O3 -fopenmp slota_madduri_par.c -o slota_madduri_par && ./slota_madduri_par
 */
...
void slota_madduri_maximal_cliques_parallel(Graph *graph, CliqueList *result) {
    ...
    CliqueList local_results[omp_get_max_threads()];
    #pragma omp parallel
    {
        int thread_id = omp_get_thread_num();
        init_clique_list(&local_results[thread_id], 1000);
        ...
        for (int idx = 0; idx < graph->num_nodes; idx++) {
            ...
            if (!visited[node]) {
                ...
                if (is_maximal_clique(graph, &clique, visited)) {
                    add_clique_to_list(&local_results[thread_id], &clique);
                }
    ...
    init_clique_list(result, 1000);
    for (int i = 0; i < omp_get_max_threads(); i++) {
        for (int j = 0; j < local_results[i].count; j++) {
            add_clique_to_list(result, &local_results[i].cliques[j]);
        }
        free_clique_list(&local_results[i]);
    }
